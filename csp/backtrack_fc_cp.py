...
class Csp:
    ...
    def make_arc_consistent(self, course1, course2):
        for ta in self.tas:
            ...
            if ta.assigned < 1 and self.matches_skills(ta, course1) and self.available(ta, course1):
                for ta1 in self.tas:
                    if ta1 != ta and (ta1.assigned < 1 and self.matches_skills(ta1, course2) and self.available(ta1, course2)):
                        inconsistent = False
                        break
                if inconsistent:
                    return 'inconsistent'
    ...
    def recursive_backtrack_fc_cp(self, assignment):
        ...
        for ta in self.tas:
            ...
            if self.ta_assignment_consistent(ta, course):
                self.add_to_assignment(assignment, ta, course)
                if not self.legal_values_for_unassigned(ta, course) or not self.constaint_propogation_check(ta, course, assignment):
                     self.remove_from_assignment(assignment, ta, course)
                result = self.recursive_backtrack_fc_cp(assignment)
                if result is not 'failure':
                    return result
                self.remove_from_assignment(assignment, ta, course)
        return assignment
