...
class Csp:
        ...
        if[value for key,value in assignment.items()]:
            for element in [value for key,value in assignment.items()]:
                for ta in element[::2]:
                    ta_set.add(ta)
            # print(ta_set)
            # tas_finished = len(ta_set) == set(self.TAList)
            tas_finished = len(set(self.TAList) - ta_set) <= 2
            # print(len(set(self.TAList) - ta_set))
            if(tas_finished):
                    return True
    ...
    def legal_values_for_unassigned(self, ta, course, assignment):
    ...
    def backtrack_search_fc(self):
        return self.recursive_backtrack_fc(collections.OrderedDict())
        # return self.recursive_backtrack_fc({})
    def recursive_backtrack_fc(self, assignment):
        ...
        for ta in self.tas:
            ...
            if self.ta_assignment_consistent(ta, course):
                ...
                if not self.legal_values_for_unassigned(ta, course, assignment):
                     self.remove_from_assignment(assignment, ta, course)
                result = self.recursive_backtrack_fc(assignment)
...
print('Performing Backtracking Search with Forward Checking...')
begin = time.time()
result = csp.backtrack_search_fc()
end = time.time()
if(result == 'failure'):
    print('No solution found!')
else:
    print('Assignments ordered by course:')
    for course in csp.courses:
        if course.name in result.keys():
            ...
            if course.ta_required > 0:
                print course.name, 'ta_required:', course.ta_required
    # print(len(result))
    print('Assignments ordered by TAs:')
    output = {}
    for key,value in result.items():
        for ta in csp.tas:
            if ta.name in value:
               if ta.name in output:
                   output[ta.name].extend([key, value[value.index(ta.name) + 1]])
               else:
                   output[ta.name] = [key, value[value.index(ta.name) + 1]]
                # print(key, ta.name, value[value.index(ta.name) + 1])
    ...
    print 'Node count:', csp.count
    print'Memory utilized:', sys.getsizeof(Course) * csp.count, 'bytes'
